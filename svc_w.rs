...
fn process_events(e:Event,listener:& TcpListener,poll:&Poll, clients:&mut HashMap<Token, TcpStream>,mut count: usize) {
  ...
  if(e.token()!=Token(0) && e.readiness().is_readable() && e.readiness().is_writable()) {

        let mut buf =[0;512];
            let mut t = e.token();
            let mut cl = clients.get_mut(&e.token()).unwrap();
            ...
            match cl.read(&mut buf){
                Ok((_)) => {
                    ...
                    if (*size==2){
                    cl.write(b"start").unwrap();    
                    let bytes_no = cl.read(&mut buf).unwrap();
